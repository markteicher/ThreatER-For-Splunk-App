...
Modular Input: Allow List Actions
...
Purpose:
- Collect allow list action events
- Track manual and automated allow list changes
- Preserve raw payloads for audit and investigation
- Support pagination, proxy, SSL verification
...
ENDPOINT = "/lists/allow/actions"
SOURCETYPE = "threater:allow_list_action"


class ThreatERAllowListActionsInput(ThreatERModularInput):
    """
    Modular input for ThreatER allow list actions
    """

    def collect(self):
        self.logger.info("Starting ThreatER allow list action collection")

        checkpoint = ThreatERCheckpoint(
            self,
            key="allow_list_actions_last_updated"
        )
        ...
        while True:
            ...
            self.logger.info(
                f"Fetched {len(records)} allow list actions "
        ...
        self.logger.info(
            f"ThreatER allow list action collection complete — "
...
def main():
    try:
        runner = ThreatERAllowListActionsInput(
            input_name="threater_allow_list_actions",
            sourcetype=SOURCETYPE
        )
        runner.run()
