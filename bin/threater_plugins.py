...
Modular Input: Plugins (External Lists)
...
Purpose:
- Collect ThreatER Plugins / External Lists
...
ENDPOINT = "/plugins"
SOURCETYPE = "threater:plugin"


class ThreatERPluginsInput(ThreatERModularInput):
    """
    Modular input for ThreatER Plugins / External Lists
    """

    def collect(self):
        self.logger.info("Starting ThreatER plugin collection")

        checkpoint = ThreatERCheckpoint(
            self,
            key="plugins_last_updated"
        )
        ...
        self.logger.info(
            f"ThreatER plugin collection complete — "
...
def main():
    try:
        runner = ThreatERPluginsInput(
            input_name="threater_plugins",
            sourcetype=SOURCETYPE
        )
        runner.run()
