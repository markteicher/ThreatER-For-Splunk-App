...
Modular Input: Allow Lists

Collects ThreatER Allow Lists
...
ENDPOINT = "/lists/allow"
SOURCETYPE = "threater:allow_list"


class ThreatERAllowListsInput(ThreatERModularInput):
    """
    Modular input for ThreatER Allow Lists
    """

    def collect(self):
        self.logger.info("Starting ThreatER allow list collection")

        checkpoint = ThreatERCheckpoint(
            self,
            key="allow_lists_last_updated"
        )
        ...
        while True:
            ...
            self.logger.info(
                f"Fetched {len(records)} allow lists "
        ...
        self.logger.info(
            f"ThreatER allow list collection complete — "
...
def main():
    try:
        runner = ThreatERAllowListsInput(
            input_name="threater_allow_lists",
            sourcetype=SOURCETYPE
        )
        runner.run()
