...
ENDPOINT = "/subscriptions/unexpected-blocks"
SOURCETYPE = "threater:unexpected_block"


class ThreatERUnexpectedBlocksInput(ThreatERModularInput):
    """
    Modular input for ThreatER unexpected blocks
    """

    def collect(self):
        self.logger.info("Starting ThreatER unexpected block collection")

        checkpoint = ThreatERCheckpoint(
            self,
            key="unexpected_blocks_last_updated"
        )
        ...
        while True:
            ...
            self.logger.info(
                f"Fetched {len(records)} unexpected blocks "
        ...
        self.logger.info(
            f"ThreatER unexpected block collection complete — "
...
def main():
    try:
        runner = ThreatERUnexpectedBlocksInput(
            input_name="threater_unexpected_blocks",
            sourcetype=SOURCETYPE
        )
        runner.run()
