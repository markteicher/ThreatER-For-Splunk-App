...
Purpose:
- Collect policy port definitions
- Capture inbound / outbound port enforcement
...
ENDPOINT = "/policies/ports"
SOURCETYPE = "threater:policy_port"


class ThreatERPolicyPortsInput(ThreatERModularInput):
    """
    Modular input for ThreatER policy ports
    """

    def collect(self):
        self.logger.info("Starting ThreatER policy port collection")

        checkpoint = ThreatERCheckpoint(
            self,
            key="policy_ports_last_updated"
        )
        ...
        while True:
            ...
            self.logger.info(
                f"Fetched {len(ports)} policy ports "
        ...
        self.logger.info(
            f"ThreatER policy port collection complete — "
...
def main():
    try:
        runner = ThreatERPolicyPortsInput(
            input_name="threater_policy_ports",
            sourcetype=SOURCETYPE
        )
        runner.run()
