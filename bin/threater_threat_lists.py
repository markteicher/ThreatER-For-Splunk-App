...
Modular Input: Threat Lists

Collects ThreatER Threat Lists
...
ENDPOINT = "/lists/threat"
SOURCETYPE = "threater:threat_list"


class ThreatERThreatListsInput(ThreatERModularInput):
    """
    Modular input for ThreatER Threat Lists
    """

    def collect(self):
        self.logger.info("Starting ThreatER threat list collection")

        checkpoint = ThreatERCheckpoint(
            self,
            key="threat_lists_last_updated"
        )
        ...
        while True:
            ...
            self.logger.info(
                f"Fetched {len(records)} threat lists "
        ...
        self.logger.info(
            f"ThreatER threat list collection complete — "
...
def main():
    try:
        runner = ThreatERThreatListsInput(
            input_name="threater_threat_lists",
            sourcetype=SOURCETYPE
        )
        runner.run()
